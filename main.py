...
if st.sidebar.button("Gerar Novos Clientes e Alocar Mesas"):
    ...
    for i in range(num_clientes_input):
        ...
        if random.random() < prob_pagamento_exato:
            ...
            # MOEDAS_DISPONIVEIS: [100, 50, 20, 5, 1]
            # Denominações únicas no intervalo [1,39]: 1, 5, 20.
            # Probabilidade de troco de denominação única (se não for 0): 3/39 = ~7.7%
            # Portanto, aproximadamente (1-0.05) * (3/39) = 0.95 * 0.077 = ~7.3% de chance de troco de denominação única.
            # E 5% de chance de troco 0.
            # Isto parece suficientemente variado.
...
with col_pagamento:
    ...
    if cliente_pag:
        ...
        st.markdown(f"**Valor da Conta:** `R$ {cliente_pag['valor_conta']}`") # Sem .2f
        st.markdown(f"**Valor Oferecido:** `R$ {cliente_pag['pagamento_oferecido']}`") # Sem .2f
        ...
        if troco_devido < 0:
            ...
            st.markdown(f"**Troco a ser dado:** `R$ {troco_devido}`") # Sem .2f
        ...
        if troco_devido >= 0:
            if st.button(f"✅ Confirmar Pagamento de {cliente_pag['id']} e Continuar Simulação", key=f"confirm_pag_{cliente_pag['id']}"):
                ...
                st.session_state.cliente_pagando_atual = None
                st.session_state.troco_ideal_calculado_formatado = None # Limpar o troco formatado
                ...
                if st.session_state.clientes_esperando_na_fila_pagamento:
                    ...
                    if any(c['status'] != 'finalizado' for c in st.session_state.clientes): # Verifica se ainda há clientes não finalizados
                         st.session_state.simulacao_ativa = True 
                         st.session_state.log_eventos.append("Todos os pagamentos pendentes resolvidos. Simulação retomada.")
                    else:
                        st.session_state.simulacao_ativa = False # Mantém pausado se todos finalizaram
